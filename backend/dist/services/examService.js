...
export class ExamService {
    // ================= EXAM TYPE METHODS =================
    static async createExamType(data) {
    ...
    static async getExamsByClass(classId, academicYearId) {
    ...
    static async getExamsForTeacher(teacherUserId, academicYearId) {
    ...
    // ================= MARKS ENTRY METHODS =================
    static async getStudentsForMarksEntry(examId) {
    ...
    static async enterMarks(examId, marks, enteredBy) {
        const exam = await prisma.exam.findUnique({
            where: { id: examId }
        });
        if (!exam) {
    ...
    static async lockExam(examId, lockedBy) {
        const exam = await prisma.exam.findUnique({
            where: { id: examId }
        });
        if (!exam) {
    ...
    static async unlockExam(examId) {
        const exam = await prisma.exam.findUnique({
            where: { id: examId }
        });
        if (!exam) {
    ...
    static async getStudentExamResults(studentId, academicYearId) {
        const where = {
            studentId
        };
        if (academicYearId) {
