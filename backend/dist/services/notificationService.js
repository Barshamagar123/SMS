...
import { sendNotificationToUser, updateUnreadCount } from '../config/socket.js';
const prisma = new PrismaClient();
export class NotificationService {
    // Send to single user
    static async sendToUser(userId, title, message, type, relatedId) {
    ...
    static async sendToMany(userIds, title, message, type, relatedId) {
        if (userIds.length === 0)
            return [];
    ...
    static async sendToClass(classId, title, message, type, relatedId) {
    ...
    static async sendToRole(role, title, message, type, relatedId) {
