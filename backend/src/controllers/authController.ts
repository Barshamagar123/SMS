...
class AuthController {
  ...
  createTeacher = async (req: AuthenticatedRequest, res: Response) => {
    try {
  ...
  // ================= PUBLIC REGISTER =================
  publicRegister = async (req: any, res: Response) => {
    try {
  ...
  // ================= APPROVE / REJECT USER =================
  approveOrRejectUser = async (req: any, res: Response) => {
    try {
      const { userId, action } = req.body;

      const data = await AuthService.approveOrRejectUser(
        Number(userId),
        action
      );
