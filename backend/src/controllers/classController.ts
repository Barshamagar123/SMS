...
// ============================================
export const getSubjectsForClass = async (req: AuthenticatedRequest, res: Response) => {
    try {
...
// ============================================
export const assignSubjectToClass = async (req: AuthenticatedRequest, res: Response) => {
    try {
        const classId = toInt(req.params.classId);
        ...
        let status = 500;
        if (error.message.includes('not found')) status = 404;
        else if (error.message.includes('already assigned')) status = 409;
...
// ============================================
export const removeSubjectFromClass = async (req: AuthenticatedRequest, res: Response) => {
    try {
        const classId = toInt(req.params.classId);
        ...
        else if (error.message.includes('Cannot remove')) status = 400;
