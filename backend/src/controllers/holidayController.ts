...
export const addHoliday = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    await NotificationService.sendToRole(
      'STUDENT',
      '🏖️ Holiday Announcement',
      `School will remain closed on ${new Date(date).toLocaleDateString()} for ${name}.`,
      'HOLIDAY',
      holiday.id
    );
    ...
    await NotificationService.sendToRole(
      'ADMIN',
      '🏖️ Holiday Added',
      `Holiday "${name}" has been added for ${new Date(date).toLocaleDateString()}.`,
      'HOLIDAY',
      holiday.id
    );
