...
export class StudentAssignmentService {
  ...
  static async createAssignment(data: {
    ...
    await NotificationService.sendToClass(
      ...
      `New assignment "${data.title}" has been posted. Due date: ${new Date(data.dueDate).toLocaleDateString()}`,
      'ASSIGNMENT_CREATED',
      assignment.id
    );
  ...
  static async getStudentAssignments(studentId: number) {
  ...
  static getAssignmentStatus(assignment: any, submission: any): string {
    ...
    if (new Date(assignment.dueDate) < new Date()) return 'LATE';
    return 'PENDING';
  }

  static async getAssignmentById(assignmentId: number, studentId?: number) {
  ...
  static async submitAssignment(
    ...
    await NotificationService.sendToUser(
      ...
      `${student?.user.name} has submitted "${assignment.title}".`,
  ...
  static async gradeSubmission(submissionId: number, marksObtained: number, feedback?: string, teacherId?: number) {
    ...
    await NotificationService.sendToUser(
      ...
      '✅ Assignment Graded',
      `Your "${submission.assignment.title}" has been graded. Score: ${marksObtained}/${submission.assignment.totalMarks} (${grade})`,
      'ASSIGNMENT_GRADED',
  ...
  static async getTeacherAssignments(teacherId: number) {
    const assignments = await prisma.assignment.findMany({
      where: { 
        teacherId, 
        isActive: true 
  ...
  static async deleteAssignment(assignmentId: number, teacherId: number) {
